...
class PlayerReservation(APITestCase):
    ...
    def test_get_reservation_list(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            User.objects.filter(is_superuser=False).count(), len(response.data)
        )
...
class PikeCountInStats(APITestCase):
    ...
    def setUp(self):
        ...
        PlayersInTeam.objects.create(
            season=currentSeason, team=team1, player=user1, is_captain=True
        )
        user2 = User.objects.create_user("test@captain.2", "test@captain.2", "password")
        PlayersInTeam.objects.create(
            season=currentSeason, team=team2, player=user2, is_captain=True
        )

        for i in range(1, 9):
            u = User.objects.create_user(
                "test@user." + str(i), "test@user." + str(i), "password"
            )
            if i < 5:
                team = team1
            else:
                team = team2
            PlayersInTeam.objects.create(
                season=currentSeason, team=team, player=u, is_captain=False
            )
