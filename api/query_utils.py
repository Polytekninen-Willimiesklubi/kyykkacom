...
from django.db.models import Count, Q, F, FloatField, Value, IntegerField, Case, When, SmallIntegerField
...
def count_field_total(val: int | str, *fields: str, gte: bool = False) -> Count | Literal[0]:
...
def rounded_divison(numer_field: str, denom_field: str, percentage: bool = False) -> Round | Value:
    """Returns the rounded (2 decimals) division of numerator by denominator. 
    If demominator is zero, returns NaN.
    """
    return Round(
        F(numer_field) / F(denom_field) * 100 if percentage else F(numer_field) / F(denom_field),
        precision=2,
        output_field=FloatField(),
    ) if F(denom_field) else Value("NaN")
...
def scores_casted_to_int() -> dict[str, Cast]:
    ...
    return {
        cast: Cast(F(org), IntegerField()) for cast, org in zip(casted_score_fields, score_fields)
    }
