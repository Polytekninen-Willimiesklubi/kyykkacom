<template>
  <v-card :title="props.title" class="ma-5" style="align-self: start">
        ...
        <v-tooltip v-if="column.tooltip" :text="column.tooltip" location="top">
          <template #activator="{ props }">
            <div class="v-data-table-header__content" v-bind="props">
              ...
              <span v-if="column.align !== 'left' && props.sortable" style="width: 14px" />
              <span @click="() => toggleSort(column)">{{ column.title }}</span>
              <v-icon
                v-if="column.sortable && props.sortable"
