...
export default {
  ...
  computed: {
    filtteredItems () {
      const arr = this.sort_games_switch == 'Erittäin' ? this.matches_periods : this.matches_match
  ...
  methods: {
      ...
      this.$http
        ...
        .then(
          function (data) {
            ...
            const selectSeason = sessionStorage.season_id
            ...
            const index = all_seasons.map(ele => ele.id).indexOf(selectSeason)
    ...
    chanceSeason (value, row) {
      ...
      if (this.current_selection.includes(value.season)) { // Remove clicked season from datas
        ...
        const selected_season = this.all_seasons[index]

        index = this.colors.indexOf('') // First valid color
        this.colors[index] = value.season
        const color = this.all_colors[index]
    ...
    returnStyle (value) {
      const index = this.colors.indexOf(value.season)
      if (index === -1) {
        return
      }
    ...
    returnHeaderColor (value) {
      const index = this.column_colors.indexOf(value)
      if (index === -1) {
        return
      }
      return this.styles[index]
    ...
    filtterItems (arr, all_time_vs_seasons, search) {
      const returning_arr = all_time_vs_seasons == 'Kaikki kaudet'
        ? arr
