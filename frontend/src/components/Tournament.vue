<template>
  <div v-if="loaded_undefined" class="d-flex">
    <div align="center">
      <h1>TBD</h1>
    </div>
  </div>
...
<script setup>
  ...
  function putTeamsPlayoffBracket() {
    props.bracket_placements.forEach((bracket, idx) => {
      bracket.forEach((team) => {
        const [teamName, seed] = !props.format_2025
          ? team
          : [team.current_abbreviation, team.bracket_placement];
        let seedString;
        if (seed === null) {
          return;
        }
        ...
        const found = rounds.find(
          (ele) => ele.player1.name === seedString || ele.player2.name === seedString,
        );

        if (found) {
          const correctBracket = found.player1.name === seedString ? found.player1 : found.player2;
  ...
  function resolveGames() {
    const playoff_games = props.played_games.filter(
      (ele) => ele.post_season || ele.match_type >= 32,
    );
  ...
  function splitFirstRound() {
    ...
    for (const [key, el] of Object.entries(games)) {
      const match = st_round.value.find(
        (e) => e.player1.name == Object.keys(el)[0] || e.player2.name == Object.keys(el)[0],
      );
      if (match === undefined) {
      ...
      const [winner, loser] =
        el[match.player1.name] > el[match.player2.name]
      ...
      loser.winner = false;
      match.other_info =
        el[match.player1.name].toString() + ' - ' + el[match.player2.name].toString();
      if (el[match.tie]) {
    ...
    winners.forEach((winner, i) => {
      ...
      const new_match = rounds.find(
        (el) => el.player1.name == placementString || el.player2.name == placementString,
      );
      const correct_column = new_match.player1.name.includes(placementString)
        ? 'player1'
        : 'player2';
  ...
  function resolvePlayoffs() {
    ...
    reversed_list.forEach((ele, i) => {
      for (const [key, el] of Object.entries(ele)) {
        if (props.first_round && props.first == 9 - i) {
          continue;
        }
        const match = data.value.find(
          (e) =>
            e.type == 9 - i &&
            (Object.keys(el)[0] == e.player1.name || Object.keys(el)[0] == e.player2.name),
        );
        if (match === undefined) {
          console.log(
            "Didn't find correct match. type: " +
              (9 - i).toString() +
              ' element: ' +
              Object.keys(el),
          );
        ...
        if (team1Wins !== team2Wins) {
          const [winner, loser] =
            team1Wins > team2Wins ? [match.player1, match.player2] : [match.player2, match.player1];
          const new_winner = structuredClone(toRaw(winner));
          ...
          if (9 - i >= 4) {
            // Ignore Bronze and Finals
            if (9 - i == 4) {
              // SemiFinals -> Winner needs to be assigned to Finals
              const finals = rounds.find((ele) => ele.type === 2);
              const correct_column = finals.player1.name.includes(match.name)
                ? 'player1'
                : 'player2';
            ...
            const correct_column = new_match.player1.name.includes(match[n])
              ? 'player1'
              : 'player2';
            const template = new_match[correct_column].template_name;
