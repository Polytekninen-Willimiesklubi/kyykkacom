...
const rounds = ref([]);
const data = ref([]);
const bracket_matches = ref([]);
const st_round = ref([]);
const loaded_undefined = ref(false);

function putTeamsPlayoffBracket() {
  if (props.non_default_seeds !== undefined) {
    props.non_default_seeds.forEach(ele => {
      if (ele[1] !== undefined) {
        for (let matchIdx = 0; matchIdx < rounds.value.length; matchIdx++) {
          const placementString = (ele[1]).toString() + '. Seed'
          const match = rounds.value[matchIdx]
...
function resolveGames() {
  bracket_matches.value = props.played_games.filter(ele => !ele.post_season)
...
function loadRounds() {
  ...
  rounds.value = structuredClone(toRaw(props.rounds_parrent))
  rounds.value.forEach(ele => {
...
function splitFirstRound() {
  ...
  winners.forEach((winner, i) => {
    ...
    const new_match = rounds.value.find(el => el.player1.name == placementString || el.player2.name == placementString)
  ...
  data.value = rounds.value.filter(e => e.type != props.first)
}
      ...
      if (el[match.player1.name] !== el[match.player2.name]) { // Not a tie
        ...
        if (7 - i >= 4) { // Ignore Bronze and Finals
          if (7 - i == 4) { // SemiFinals -> Winner needs to be assigned to Finals
            const finals = rounds.value.find(ele => ele.type === 2)
          ...
          const new_match = rounds.value.find(ele => ele.id === match.next)
