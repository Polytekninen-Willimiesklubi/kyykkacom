...
export default {
  ...
  methods: {
    ...
    resolveGames () {
      ...
      const playoff_games = this.played_games.filter(ele => ele.post_season || ele.match_type >= 32)
      ...
      playoff_games.forEach(ele => {
        if (ele.match_type >= 2 & ele.match_type < 10) {
          const round = this.tmp_rounds[ele.match_type - 2]
    ...
    splitFirstRound () {
      ...
      const first_round_matches = this.rounds.filter(e => e.type == this.first)
      this.st_round = first_round_matches
      const games = this.tmp_rounds[this.first - 2]
      const winners = []
      for (const [key, el] of Object.entries(games)) {
        const match = this.st_round.find(e => e.player1.name == Object.keys(el)[0] || e.player2.name == Object.keys(el)[0])
        if (match === undefined) {
      ...
      winners.forEach((winner, i) => {
        ...
        const new_match = this.rounds.find(el => el.player1.name == placementString || el.player2.name == placementString)
    ...
    resolvePlayoffs () {
      const reversed_list = this.tmp_rounds.reverse()
      reversed_list.forEach((ele, i) => {
        for (const [key, el] of Object.entries(ele)) {
          if (this.first_round && this.first == 7 - i) { continue }
          const match = this.data.find(e => e.type == 7 - i && (Object.keys(el)[0] == e.player1.name || Object.keys(el)[0] == e.player2.name))
          if (match === undefined) {
          ...
          if (el[match.player1.name] !== el[match.player2.name]) { // Not a tie
            ...
            if (7 - i >= 4) { // Ignore Bronze and Finals
              if (7 - i == 4) { // SemiFinals -> Winner needs to be assigned to Finals
                const finals = this.rounds.find(ele => ele.type === 2)
              ...
              const new_match = this.rounds.find(ele => ele.id === match.next)
