  ...
  methods: {
    ...
    splitToBrackets () {
      ...
      if (sessionStorage.all_seasons) {
        ...
        const this_season = all_seasons[index]
    ...
    validateResult (type) {
      const post_data_key = type ? 'bracket_placement' : 'super_weekend_bracket_placement'
      const team_set = type ? this.teams : this.super_teams

      if (confirm('Oletko tyytyväinen tuloksiin?')) {
        team_set.forEach(ele => {
          ele.forEach(e => {
            console.log(post_data_key)
            const post_url = 'api/kyykka_admin/team/update/' + e.id
            const post_data = {}
    ...
    validateSeeds () {
      if (confirm('Oletko tyytyväinen tuloksiin?')) {
        this.seedable_super_teams.forEach(ele => {
          const post_url = 'api/kyykka_admin/team/update/' + ele.id
          const post_data = { super_weekend_playoff_seed: ele.order }
          this.$http.patch(post_url, post_data, {
            headers: {
              'X-CSRFToken': this.getCookie('csrftoken')
            },
    ...
    validateWinner () {
      ...
      if (confirm('Oletko tyytyväinen runkosarjan voittajaan?')) {
        const post_url = 'api/kyykka_admin/team/update/' + this.selectValue.id
        const post_data = { bracket_placement: 0 }
        this.$http.patch(post_url, post_data, {
          headers: { 'X-CSRFToken': this.getCookie('csrftoken') },
          withCredentials: true
        }).catch(function (response) {
    ...
    async validateSuperWinner () {
      ...
      if (confirm('Oletko tyytyväinen Superin voittajaan?')) {
        const post_url = 'api/kyykka_admin/superweekend/' + this.super_weekend_id
        const post_data = { winner: this.superWinnerSelected.id }
        this.$http.patch(post_url, post_data, {
          headers: { 'X-CSRFToken': this.getCookie('csrftoken') },
          withCredentials: true
        }).catch(function (response) {
    ...
    validateBrackets () {
      if (confirm('Oletko tyytyväinen Superin lohkoihin?')) {
        this.super_teams.forEach((ele, index) => {
          ele.forEach((e) => {
            const post_url = 'api/kyykka_admin/team/update/' + e.id
            const post_data = { super_weekend_bracket: index + 1 }
            this.$http.patch(post_url, post_data, {
              headers: {
                'X-CSRFToken': this.getCookie('csrftoken')
              },
        ...
        this.not_in_super.forEach(ele => {
          const post_url = 'api/kyykka_admin/team/update/' + ele.id
          const post_data = { super_weekend_bracket: null }
          this.$http.patch(post_url, post_data, {
            headers: {
              'X-CSRFToken': this.getCookie('csrftoken')
            },
    ...
    async submit () {
      ...
      const match_time = this.selectDate + ' ' + this.selectTime + ':00'
