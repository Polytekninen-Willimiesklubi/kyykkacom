<template>
  <v-app-bar style="z-index: 2" color="grey-darken-3">
    <router-link
      to="/"
      style="text-decoration: none; color: white; padding-right: 2em; padding-left: 2em"
    >
      <img src="@/assets/kyykkalogo120px.png" />
    </router-link>

    <template v-for="item in headersNavBar">
      <v-btn v-if="item.title === 'Info'" class="hidden-md-and-down" append-icon="mdi-menu-down">
        {{ item.title }}
        <v-menu activator="parent" :open-on-hover="true" color="grey-darken-3">
    ...
    <div v-if="!userStore.loggedIn" class="hidden-md-and-down pa-4">
      <log-in />
      <register />
    </div>
    ...
    <div v-else class="hidden-md-and-down">
      <v-btn
        v-if="userStore.teamId"
        text="Oma Joukkue"
        :to="{ path: `/joukkueet/${userStore.teamId}` }"
      />
      <v-btn v-else text="Ei joukkuetta" />
      <v-btn :text="userStore.playerName" :to="'/pelaajat/' + userStore.userId" />
      <v-btn text="Kirjaudu Ulos" :to="'/'" @click="userStore.logOut()" />
    </div>
  ...
  >
    <v-list :nav="true">
      <template v-for="item in headersNavBar">
        <v-list-item
          v-if="(item.title !== 'Oma Joukkue' || userStore.teamId) && item.title !== 'Info'"
      ...
      />
      <v-list-item to="/info/saannot" prepend-icon="mdi-book-open-variant" title="Säännöt" />
      <v-list-item to="/info/hof" prepend-icon="mdi-trophy-variant" title="Hall of Fame" />
      <v-list-item to="/info/links" prepend-icon="mdi-link-variant" title="Muualla kyykkää" />

      <v-list-item>
        <div v-if="!userStore.loggedIn" class="pa-2">
          <log-in />
          <register />
        </div>
        <v-btn
    ...
    <template v-if="userStore.loggedIn" #prepend>
      <v-list-item>
