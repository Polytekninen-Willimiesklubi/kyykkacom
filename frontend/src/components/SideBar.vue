  ...
  <v-card>
    <v-card-title>
      <v-row>
        ...
        <v-col v-if="!disable_close" cols="1" align="right" class="close_button">
          <v-tooltip location="top" text="Piilota Taulukko">
    ...
    <v-row v-for="(listItem, index) in teams" :key="index">
      <v-col>
        ...
        >
              ...
              <template v-for="column in columns" :key="column.key">
                <th class="v-data-table__td v-data-table__th">
                  <div class="v-data-table-header__content">
                    <span
                      :style="
                        boldingKeys && column.title === boldingKeys[0] ? 'font-weight:bold' : ''
                      "
                    >
                      {{ column.title }}
                    </span>
          ...
          <template #item="{ item, index }">
            ...
            >
              <td
                ...
                :style="boldingKeys && header.key === boldingKeys[1] ? 'font-weight:bold' : ''"
              >
                {{ item[header.key] }}
              </td>
            </tr>
          ...
          <template #bottom />
          <!-- This disables paginations menu -->
        </v-data-table>
      </v-col>
    </v-row>
...
<script setup>
  ...
  function isSecond(val) {
    ...
    const index = props.teams.length > 1 ? 1 : 0;
    return (
      (props.lines && props.lines.length === 1 && props.lines[index][0] === val + 1) ||
