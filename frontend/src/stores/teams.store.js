...
export const useTeamsStore = defineStore('joukkue', () => {
  ...
  const seasonTeamAccolades = ref(
    JSON.parse(localStorage.getItem('seasonTeamAccolades'))
      ? JSON.parse(localStorage.getItem('seasonTeamAccolades'))
      : {},
  );
  const secondStage = ref(
    JSON.parse(localStorage.getItem('secondStage'))
      ? JSON.parse(localStorage.getItem('secondStage'))
      : [],
  );
  ...
  const seasonStats = computed(() => {
    ...
    const returnValue =
      Object.keys(seasonsStats.value).length && selectedSeasonId.value
