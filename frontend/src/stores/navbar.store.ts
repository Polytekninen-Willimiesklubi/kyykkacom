...
export const useNavBarStore = defineStore('navbar', () => {
    ...
    const playoffLines = computed<number[][]>(
        () => currentSeasonMapping.value?.playoffLines ?? []
    );
    ...
    async function getSeasons(): Promise<void> {
        ...
        try {
            ...
            let allSeasonsTmp: Season[] = getLocal('allSeasons') || [];
            if (!allSeasonsTmp || allSeasonsTmp.length === 0) {
                let tmpPayload = await fetchWrapper(baseUrl, {}, 'GET', true);
