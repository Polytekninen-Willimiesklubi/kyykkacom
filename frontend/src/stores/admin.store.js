...
export const useAdminStore = defineStore('admin', () => {
  ...
  async function getData() {
    ...
    regularSeasonTeams.value = structuredClone(
      toRaw(teamStore.bracketedTeams).map((ele) => ele.map(toRaw)),
    );
    superWeekendTeams.value = structuredClone(
      toRaw(superStore.bracketedTeams).map((ele) => ele.map(toRaw)),
    );
    flattenSuperTeams.value = structuredClone(toRaw(superWeekendTeams.value).flat());
    const jotain = teamStore.allTeams.filter(
      (ele) => !flattenSuperTeams.value.find((team) => ele.id === team.id),
    );
