...
export default {
  ...
  created () {
    ...
    const promise1 = this.$http.get('api/matches/?season=' + sessionStorage.season_id).then(
      data => games = data.body.filter(ele => ele.match_type < 30)
    )

    const promise2 = this.$http.get('api/teams/?season=' + sessionStorage.season_id + '&post_season=0').then(
      function (data) {
        for (let i = 0; i < no_brackets; i++) {
          tmp.push([])
        }
    ...
    Promise.allSettled([promise1, promise2]).then(() => {
      if (this_season !== undefined && this_season.playoff_format && this_season.playoff_format != 0) {
        const json = this.seasons_mapping[this_season.playoff_format]
