<template>
  <div class="flex-1-1-100 ml-20" style="max-width: 90%">
    <v-card>
      <v-skeleton-loader class="ma-5" :loading="teamStore.singleLoading" type="image, actions">
        <v-card elevation="0" class="ma-2">
          ...
          >
            <template #default="{ items }">
              <v-row>
                <v-col v-for="(item, i) in items" cols="6">
                  <v-row>
                    <v-col cols="6">
                      {{ item.raw.title }}
                    </v-col>
                  ...
                  <v-divider v-if="i - 1 != items.length" vertical />
                </v-col>
              </v-row>
            </template>
          </v-data-iterator>
      ...
      <v-expansion-panels v-model="panel" multiple>
        <v-expansion-panel title="Pelaajat">
          ...
          <v-spacer />
          <v-expansion-panels>
            <v-expansion-panel v-if="teamStore.reserveAllowed" title="Varaa pelaajia">
              <v-expansion-panel-text>
                <v-text-field
                  v-model="search"
                  class="mb-10 mt-0"
                ...
                >
                  <template #item.actions="{ item }">
                    <v-icon icon="mdi-plus" color="green" @click="teamStore.reservePlayer(item)" />
                  </template>
        ...
        <v-expansion-panel v-if="teamStore.selectedSeasonId === 'allTime'" title="Saavutukset">
          <v-expansion-panel-text>
              ...
              <v-col cols="12">
                ...
                >
                  ...
                  <v-row style="overflow: hidden">
                    <v-col cols="6">
                      <template
                        v-for="accolade in achievements['team_accolades']"
                        :key="accolade.id"
                      >
                    ...
                    <v-divider
                      ...
                      inset
                      :thickness="1"
                      style="border-color: #000000 !important; color: #000000"
                    />
...
<script setup>
  ...
  if (authStore.isSuperUser || authStore.isTeamCaptain(parseInt(route.params.id))) {
