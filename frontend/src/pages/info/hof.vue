      ...
      <v-tabs-window v-model="tab">
            ...
            <template v-for="table in tab_one_data">
              <v-col :cols="table.cols">
                <simple-table
                  ...
                  :items="table.data"
                >
                  <template v-if="table.title == 'Runkosarjan Voittaja'" #bottom_legend>
                    <div class="d-flex">
                      <p class="legend_text ml-2" style="text-align: left">
                        * : Ei (ehkä) virallinen
                      </p>
                      <v-spacer />
        ...
        <v-tabs-window-item value="two">
          <v-row>
            ...
            <v-col cols="4">
              <simple-table title="Sekalaiset" :headers="headerHofMisc" :items="hof.misc" />
            </v-col>
            <v-col cols="4">
              <simple-table
          ...
          <v-card>
            <v-list lines="three">
              <template v-for="i in info">
                <v-list-item :title="i.title" :subtitle="i.subtitle">
        ...
        <v-tabs-window-item value="four">
          <v-row style="align-self: start">
            <v-col cols="12">
              <v-card title="Mitallitaulukko">
                <v-row>
                  <v-col cols="12">
                    <div class="d-flex justify-center">
                      <v-btn-toggle v-model="toggleMultiple" variant="outlined" divided mandatory>
                        <v-btn size="small" text="Joukkueet" />
                        <v-btn size="small" text="Pelaajat" />
                      </v-btn-toggle>
                    </div>
                  </v-col>
                  <v-col>
                    <v-card class="ma-5">
                      ...
                      >
                        <template
                          v-for="header in headerHofMedals"
                        ...
                        <template #item.name="{ item }">
                          <span>
                            <a
                              class="a_link_no_under"
                              :href="
                                toggleMultiple ? `pelaajat/${item.id}` : `joukkueet/${item.id}`
                              "
                            >
                              {{ item.name }}
                            </a>
              ...
              <v-card title="Ennätyksiä">
                <v-card class="ma-4" title="Yksittäisen heiton tulos">
                  ...
                  >
                    ...
                    <template #item.player_name="{ item }">
                      <span v-if="!item._empty">
                        <a class="a_link_no_under" :href="`pelaajat/${item.player_id}`">
                          {{ item.player_name }}
                        </a>
                      </span>
                      <span v-else>
                      ...
                      <tr>
                        <td :colspan="columns.length">
                          <div style="text-align: center; font-weight: bold">
                            {{ item.items[0].raw.score }} Kyykkää ({{
                              hof.records.counts[item.items[0].raw.score] || item.items.length
                            }}
                            heittoa)
                          </div>
                        </td>
                ...
                <v-card class="ma-4" title="Top 10+ Erätulos">
                  ...
                  >
                    ...
                    <template #item.team_name="{ item }">
                      <span>
                        <a class="a_link_no_under" :href="`joukkueet/${item.team_id}`">
                          {{ item.team_name }}
                        </a>
                      </span>
                    </template>
                ...
                <v-card class="ma-4" title="Top 10+ Erässä eniten pelaajan poistamia kyykkiä">
                  ...
                  >
                    ...
                    <template #item.player_name="{ item }">
                      <span>
                        <a class="a_link_no_under" :href="`pelaajat/${item.player}`">
                          {{ item.player_name }}
                        </a>
                      </span>
                    </template>
                ...
                <v-card class="ma-4" title="Top 10+ Pisimmät nollattomat heittoputket">
                  ...
                  <div class="d-flex justify-center align-center">
                    <v-btn-toggle v-model="toggleNonZero" variant="outlined" divided mandatory>
                      <v-btn size="small" text="Kaikki" value="all" />
                      <template v-for="i in 4" :key="i">
                        <v-btn size="small" :text="`${i}`" :value="i" />
                      </template>
                    </v-btn-toggle>
                  </div>
                  <v-data-table
                  ...
                  >
                    ...
                    <template #item.match_time="{ item }">
                      <span>
                        ...
                        {{
                          item.end_time
                            ? date.formatByString(date.date(item.end_time), 'dd.MM.yyyy')
                            : ''
                        }}
                      </span>
                    </template>
                    ...
                    <template #group-header="{ item, columns, toggleGroup, isGroupOpen }">
                      ...
                      <tr>
                        <td :colspan="columns.length">
                          <div
                            v-if="
                              item.items[0].raw.score_type !== 'h' &&
                              item.items[0].raw.score_type !== '0'
                            "
                          ...
                          <div
                            v-if="item.items[0].raw.score_type === '0'"
                            style="text-align: center; font-weight: bold"
                          >
                            Virkamiehiä (0 Kyykkää)
                          </div>
                          <div
                            v-if="item.items[0].raw.score_type === 'h'"
                            style="text-align: center; font-weight: bold"
                          >
                            Haukkia (Ohi heitto)
                          </div>
...
<script setup>
  import {
    ...
    headerHofMedals,
    headerHofRecords,
    headerHofRecordsPeriod,
    headerHofRecordsMatch,
    headerHofRecordsPlayerRound,
    headerHofRecordsPlayerMatch,
    headerHofRecordsPlayerNonZero,
    headerHofRecordsPlayerNonZeroCurrent,
    headerHofRecordsResultStreak,
  } from '@/stores/headers';
  ...
  const tab_one_data = [
    {
      title: 'Liigamestaruus',
      cols: 5,
      slice: [undefined, undefined],
      data: hof.championship,
      icon: 'mestari.ico',
    },
    {
      title: 'SuperWeekend-Cup',
      cols: 4,
      slice: [0, 3],
      data: hof.superData,
      icon: 'superweekend.ico',
    },
    {
      title: 'Runkosarjan Voittaja',
      cols: 3,
      slice: [0, 2],
      data: hof.bracketWinners,
      icon: 'runkomestari.ico',
    },
    { title: 'Henkkari-Cup', cols: 4, slice: [0, 3], data: hof.singleWinner, icon: null },
    {
      title: 'Parikyykkäliiga',
      cols: 4,
      slice: [0, 3],
      data: hof.pairWinners,
      icon: 'apila_palkinto.png',
    },
    { title: 'SM-Kyykkä', cols: 4, slice: [0, 3], data: hof.sm, icon: null },
    {
      title: 'Kyykkää tähtien kanssa',
      cols: 4,
      slice: [0, 3],
      data: hof.stars,
      icon: 'star_trophy.svg',
    },
  ];
  ...
  const info = [
    ...
    },
    {
      title: 'Parikyykkäliiga',
      subtitle:
        'Syksyllä pelattava Parikyykkäliiga pelataan henkkarimuotoisena kentän kyykkien osalta, mutta paripelinä. Pudotuspeli vaiheen voittajalle jaetaan Apila-pokaali. Apila-pokaalin kiertoon lahjoitti lassi onne. Liiga on järjestty vuodesta 2022 lähtien.',
    },
    {
      title: 'Kyykkää Tähtien Kanssa',
      subtitle:
        'Kevytmielisempi turnaus, missä NKL pelaaja ja NKLssä pelaamaton (usein fuksi) pelaavat parina. Järjestetään syksyisin. Järjestetty jo kauan.',
    },
    {
      title: 'SM-Kyykkä',
      subtitle:
        "Wappuna järjestettävä joukkuemuotoinen turnaus. 'S' nimessä viittaa Skinnarilaan, ei Suomeen. Kilpailu on järjestty ainakin vuodesta 2014. Tarkkaa vuotta ei nyt koodaja itse tiedä.",
    },
    {
      title: 'Skinnarilan Paras Kyykän Pelaaja (SPKP)',
      subtitle:
        "Kiertopalkinto, jonka voittaa haastamalla ja voittamalla nykyisen palkinnon haltijan ELO-henkkarissa. Palkintoon saa nimen, jos 'eläköityy' aktiivisesta pelaamisesta palkinnon kanssa.",
    },

    {
      title: 'Jaskan Karttu',
      subtitle:
        'Myönnettään henkilölle, joka on tehnyt jotain merkittävää lappeen Rantalaisen kyykän edistämiseksi. Kyseessä ei ole lähtökohtaisesti vuosittain jaettava palkinto.',
      hyperlink: 'Jaskan muistolle',
      ref: '/info/jaska',
    },
    {
      title: 'KCK-Ahti - Pysti',
      subtitle:
        'Palkinto jaetaan vuosittain sellaiselle pelaajalle, joka päättyvän kauden aikana on huomionarvoisasti vaikuttanut edustamansa liigassa pelaavan joukkueen menestykseen erittäin jalolla haukienheiton saralla. Mitä kriittisemmässä paikassa, sitä parempi. ',
      hyperlink: 'Julkilausuma',
      ref: '/info/ahti',
    },
    {
      title: 'Vuoden kyykkääjä',
      subtitle:
        'Pelaaja, joka on osoittanut kauden aikana parasta urheiluhenkeä ja herrasmiesmäistä käytöstä yhdistettynä pelin hyvään tasoon',
    },
    {
      title: 'Haukikuningas/-tar',
      subtitle: 'Runsaasti tai erittäin runsaasti haukia kauden aikana heittänyt henkilö.',
    },
    {
      title: 'Vuoden Tulokas',
      subtitle:
        'Ensimmäisen kauden pelaaja, joka on pelannut hyvällä tasolla ja osoittanut hyvää kyykkä-henkeä',
    },
    {
      title: 'Runkosarjan paras pelaaja',
      subtitle:
        'Joukkueen pelin edistäminen sekä korkea henkilökohtaisen pelaamisen taso runkosarjassa.',
    },
    {
      title: 'Pudotuspelien paras pelaaja',
      subtitle:
        'Joukkueen pelin edistäminen sekä korkea henkilökohtaisen pelaamisen taso jatkosarjassa.',
    },
    {
      title: 'Vuoden viimeistelijä',
      subtitle:
        'Paras yksittäisten kyykkien sekä vaikeiden linjojen poistaja; joukkueen kovan tuloksen viimeistelevä pelaaja sekä voittojen ratkaisija. Painoarvo varsinkin 4.paikan pelaajille.',
    },
    {
      title: 'Vuoden nais-/mieskyykkääjä',
      subtitle:
        'Kauden paras nais-/miespuolinen kyykänpelaaja, vastine Vuoden Kyykkääjän vastine. Joukkueen pelin edistäminen sekä korkea henkilökohtaisen pelaamisen taso.',
    },
    { title: 'Vuoden MVP', subtitle: 'Arvokkain/Paras pelaaja läpi kauden.' },
    {
      title: 'Vuoden Kuusenkaataja',
      subtitle: 'Eniten joulukuusia (heitto joka poistaa 6 tai enemmän kyykkää) heittänyt pelaaja.',
    },
  ];
  ...
  const streakData = computed(() => {
    ...
    const unpackked = [
      ...(hof.records?.score_type_streaks
        ? Object.values(hof.records.score_type_streaks).flat()
        : []),
    ];
