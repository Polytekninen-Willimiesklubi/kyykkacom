    ...
    <v-main class="content">
      <v-layout class="pa-10">
        ...
        <Transition name="sidebar">
          <div v-if="showSidebar" class="d-flex right align-self-start">
            <v-tabs-window v-model="tab">
              <v-tabs-window-item value="one">
                <side-bar
                  ...
                  :sort-by="[
                    navStore.selectedSeason.playoff_format === 8
                      ? { key: 'first_bracket_placement', order: 'asc' }
                      : { key: 'bracket_placement', order: 'asc' },
                ...
                >
                  <template v-if="navStore.selectedSeason.playoff_format === 8" #button>
                    <v-btn text="Jatkosarja ->" @click="tab = 'two'" />
                  </template>
                </side-bar>
              </v-tabs-window-item>
              <v-tabs-window-item v-if="navStore.selectedSeason.playoff_format === 8" value="two">
                <side-bar
                  title="Jatkosarja"
                  class="hidden-md-and-down"
                  :headers="headersDefaultSideBar"
                ...
                >
                  <template #button>
                    <v-btn text="<- Runkosarja" @click="tab = 'one'" />
                  </template>
                </side-bar>
              </v-tabs-window-item>
            </v-tabs-window>
        ...
        <Transition name="openbutton">
          <div v-if="!showSidebar" class="pr-5">
            <v-tooltip location="left" text="Avaa Sarjataulukko">
              <template #activator="{ props }">
                <v-fab v-bind="props" icon="mdi-format-list-numbered" @click="showSidebar = true" />
              </template>
            </v-tooltip>
          </div>
        </Transition>
